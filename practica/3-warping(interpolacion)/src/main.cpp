...
glm::vec3 warpPoint(const Delaunay &delaunay0, const Delaunay &delaunay1, glm::vec3 p) {
	...
	const std::vector<Triangulo> &triangulos_iniciales = delaunay0.getTriangulos();
	auto triangulo_inicial = triangulos_iniciales[index];
	
	const std::vector<glm::vec3> &puntos_iniciales = delaunay0.getPuntos();
	const std::vector<glm::vec3> &puntos_finales = delaunay1.getPuntos();
	
	auto vertices_iniciales =  triangulo_inicial.vertices;
	// Pesos primer triangulo
	auto a_inicial = puntos_iniciales[vertices_iniciales[0]];
	auto b_inicial = puntos_iniciales[vertices_iniciales[1]];
	auto c_inicial = puntos_iniciales[vertices_iniciales[2]];
	auto pesos = calcularPesos(a_inicial,b_inicial,c_inicial,p);
	
	// Nuevos vertices
	auto a_final = puntos_finales[vertices_iniciales[0]];
	auto b_final = puntos_finales[vertices_iniciales[1]];
	auto c_final = puntos_finales[vertices_iniciales[2]];
	
	auto p_final = a_final*pesos[0] + b_final*pesos[1] + c_final*pesos[2];
