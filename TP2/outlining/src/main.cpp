...
void outlineInstance(const Instance &instance, glm::vec3 color_silhouette) {
	...
	glStencilFunc(OPERACION , VALOR QUE DEJA EN EL STENCIL, ESQUIZOFRENIA ) Especifica cuando pasa el test de stencil
	
	GL_NOTEQUAL : Cuando el valor del stencil es distito de un valor
	GL_GEQUAL : Cuando el valor es mayor o igual
	GL_LEQUAL : Menor o igual
	GL_EQUAL : Igual 
	
	glStencilOp(P1,P2,P3) Especifica que hacer en cada caso
	
	P1 : Que hacer cuando falla el test de stencil
	P2 : Que hacer cuando pasa el test de stencil pero no el profundidad
	P3 : Que hacer cuando pasan los 2
	
	Valores :  GL_KEEP, GL_ZERO, GL_REPLACE, GL_INCR, GL_INCR_WRAP, GL_DECR, GL_DECR_WRAP, and GL_INVERT. The initial value is GL_KEEP.
	
	*/
	//glStencilFunc(GL_NEVER,1,0xFF);
	//glStencilOp(GL_ZERO,GL_ZERO,GL_ZERO);
	//shader_silhouette.setUniform("color",glm::vec4{color_silhouette,0.2f});
	//drawInstance(instance,shader_silhouette); 
	
	//////////////////////////////////////////////////////////////////////
	glStencilFunc(GL_NEVER,1,0xFF); // En esta seccion nunca pasa el stencil
	glStencilOp(GL_REPLACE,GL_ZERO,GL_ZERO); // Cuando falla el stencil actualiza el stencil
	glColorMask(0,0,0,0);
	drawInstance(instance,shader_texture); // Dibuja el pollo dejo valor 1
	glColorMask(1,1,1,1);
	/////////////////////////////////////////////////////////////////////
	...
	glStencilFunc(GL_NOTEQUAL,1,0xFF); // Aca decimos que lo que no sea igual a 1 en la mascara, que dibuje tranqui
	...
	drawInstance(instance,shader_silhouette); // Dibuja la capa opaca con el z activado
	glDisable(GL_DEPTH_TEST);
	...
	shader_silhouette.setUniform("outline_width",outline_width);
	shader_silhouette.setUniform("color",glm::vec4{color_silhouette,0.2f});
	drawInstance(instance,shader_silhouette); // Dibuja la capa de transparencia arriba de todo (sin z test) donde el estencil no sea 1
	/* */
...
//	view_pos.z *= 1.5;
	...
	do {
		...
		if (selected_instance!=-1) {
			...
			outlineInstance(instances[selected_instance],{1.f-s,s,0.f});
		}
