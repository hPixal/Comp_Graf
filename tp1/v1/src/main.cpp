...
#define VERSION 20230522

Shader shader_toon;
bool rotate = true, enable_toon = false;
...
int main() {
	...
	shader_toon = Shader("shaders/phong"); // phong, por si toon no compila
	bool shader_ok = reload_shader_toon();
	
	// model
	auto models = Model::load("homer", Model::fNoTextures);
	// material
	glm::vec3 mat_color = {1.f,0.f,0.f};
	...
	// misc
	view_target.x = -0.40;
	glm::vec3 background_color = {0.9f,0.9f,0.7f};
	...
	do {
		...
		if (enable_toon) {
			...
			for(auto &model : models) {
				if (glm::dot(model.material.kd, model.material.kd)<0.1f) continue; // no "engrosar" los pelos de homero
		...
		shader.setUniform("specular_strength",mat_spec_strength);
		glCullFace(GL_BACK);
		for(Model &model : models) {
			shader.setLight(light_pos, light_color, ambient_strength);
			shader.setBuffers(model.buffers);
			shader.setUniform("color", model.material.kd*1.75f);
			model.buffers.draw();
		}
...
bool reload_shader_toon() {
	try {
		...
		shader_toon = std::move(new_shader);
		return true;
