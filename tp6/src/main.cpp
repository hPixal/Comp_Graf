...
void updateControlPointsAround(Spline &spline, int ctrl_pt) {
	...
	glm::vec3 actual = spline.getControlPoint(ctrl_pt);
	glm::vec3 anterior = spline.getControlPoint(ctrl_pt-grado);
	glm::vec3 posterior = spline.getControlPoint(ctrl_pt+grado);
	
	glm::vec3 deriv_dir = (posterior-anterior)*(1.f/2.f);
	
	spline.setControlPoint(ctrl_pt+1,actual+(deriv_dir*(1.f/grado)));
	spline.setControlPoint(ctrl_pt-1,actual-(deriv_dir*(1.f/grado)));
...
/// @todo: obtener los ejes y la nueva posicion del origen en funcion
	...
	glm::vec3 deriv;
	glm::vec3 move = spline.at(t,deriv);
	//float ang = acos(dot(normalize(deriv),{1,0,0}));
	float ang = atan2(deriv.z,deriv.x);

	float scl = 0.3f;

	glm::vec3 e_x(cos(ang),0.f,sin(ang));
	glm::vec3 e_y(0.f,1.f,0.f);
	glm::vec3 e_z(-sin(ang),0.f,cos(ang));
	glm::vec3 pos(move.x,0.f,move.z);

	// armar la matriz
	glm::mat4 m(1.f);
	m[0] = glm::vec4(scl*e_x,0.f);
	m[1] = glm::vec4(scl*e_y,0.f);
	m[2] = glm::vec4(scl*e_z,0.f);
